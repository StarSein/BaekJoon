 ...
 * 3. A 표현값에 a가 표함되어 있다면 1~10진수의 가능성은 배제해야 한다. 만약 포함하게 되면 로직 에러 발생 가능하다
 */
public class Main {
    ...
    static void solve() {
        ...
        int minA = IntStream.of(numA).min().orElse(0);
        int minB = IntStream.of(numB).min().orElse(0);

        String answer = null;
        for (int a = minA + 1; a <= 36; a++) {
            for (int b = minB + 1; b <= 36; b++) {
                if (a == b) continue;
